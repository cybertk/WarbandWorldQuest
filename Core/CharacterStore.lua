...
local function CharacterComparator(lhs, rhs, field)
	...
	local subField = Cache.sortField

	if lhs[subField][field] == rhs[subField][field] then
		return 0
	elseif lhs[subField][field] == nil then
		return -1
	elseif rhs[subField][field] == nil then
		return 1
	end

	return lhs[subField][field] < rhs[subField][field] and -1 or 1
end
